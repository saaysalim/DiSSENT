...
import com.crossbordersystem.securecomm.util.PemUtils;
...
/**
 * Wires the Fabric Gateway Java SDK using the checked-in connection profile + identity
 * (cert.pem/key.pem). The connection profile's peer/CA addresses must match wherever
 * the local test-network is actually running — see the deployment runbook.
 */
@Configuration
